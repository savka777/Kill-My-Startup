...
export async function POST_DISABLED(req: Request) {
  try {
    ...
    // Aggregate hourly metrics for the time range
    const endTime = new Date()
    const startTime = subHours(endTime, days * 24)
    
    for (const entity of entities) {
      // Get all mentions for this entity in the time range
      const allMentions = await prisma.conversationMention.findMany({
        where: {
          OR: [
            { projectId: entity.isProject ? entity.id : null },
            { competitorId: entity.isProject ? null : entity.id }
      ...
      const mentionsByHour = new Map<string, typeof allMentions>()
      
      for (const mention of allMentions) {
        const hour = startOfHour(mention.publishedAt).toISOString()
        if (!mentionsByHour.has(hour)) {
          mentionsByHour.set(hour, [])
        }
        mentionsByHour.get(hour)!.push(mention)
      }

      // Process each hour
      for (const [hourKey, hourMentions] of mentionsByHour) {
        const currentHour = new Date(hourKey)
